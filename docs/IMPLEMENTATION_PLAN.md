...
- **Known gap surfaced, not yet fixed: the live in-season trained-model path is unwired.** `predict_points()`'s trained-model branch needs `player_histories` (per-player match history via `fpl_api.get_element_summary`) fed through `features/build_features.build_current_features()`. No caller has ever built that `player_histories` dict from a real API response, and `build_current_features()` → `collapse_to_gameweek()` unconditionally references `name`/`position`/`team` columns that aren't present in a raw element-summary payload — it would raise `KeyError` if actually invoked live today. `weekly_pipeline.predict_horizon_points()` guards this explicitly: it raises `NotImplementedError` with a clear message rather than crash confusingly or silently mispredict, once completed gameweeks reach `model.cold_start_gw_threshold` (config, currently 4) and a trained model is registered. **This must be fixed before GW `cold_start_gw_threshold + 1`** of the live season — i.e. before real transfer/captaincy recommendations are needed past pre-season priors. Until then, `run_live()`'s cold-start branch (accurate for the current pre-season state) is the only exercised live path.
